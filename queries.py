...
def update_product_price(cursor):
    query = "UPDATE product SET product.Price = product.Price * 1.1;"
    cursor.execute(query)
...
def authenticate_employee(cursor, entered_id):
    query = "SELECT EmployeeID FROM employee;"
    cursor.execute(query)
    employee_ids = [str(x[0]) for x in cursor]

    if entered_id in employee_ids:
        return True
    else:
        return False            
...
def main():
    ...
    while not authenticated:
        entered_id = input("Please enter your employee ID: ")
        authenticated = authenticate_employee(cursor, entered_id)
